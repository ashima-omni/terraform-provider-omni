...
| TC-03b | `omni_user` with `attributes` | Declared keys round-trip | Not exercised: user_attribute_key left blank in CI | NOT RUN |
...
| TC-08b | `omni_connection` (opt in) | Created, ID returned | Not exercised: needs live warehouse credentials | NOT RUN |
...
| TC-09b | Plan after an out-of-band UI edit | Drift detected and reported | Needs a manual UI edit mid-run | NOT RUN |
...
| TC-13b | Change a user attribute value | In place, undeclared attributes untouched | Not exercised: no attribute key configured | NOT RUN |
| TC-14 | Rename group | In place, membership preserved | In place via SCIM PATCH. PUT is rejected by the API, see finding 13 | PASS |
| TC-14b | Remove the only group member | Membership emptied, group survives | Not exercised | NOT RUN |
...
| TC-18b | Change `omni_connection.base_role` | In place, no replacement | Not exercised: connection resource not run | NOT RUN |
| TC-18c | Change `omni_connection.host` | Forces replacement (expected) | Not exercised: connection resource not run | NOT RUN |
...
| ID | Case | Expected | Actual | Status |
| --- | --- | --- | --- | --- |
| TC-20 | Import a folder by ID | State populated, following plan clean | Not exercised in CI | NOT RUN |
...
| TC-22 | Destroy non-empty folder without `delete_recursively` | Fails with a clear API error | Needs a manual run without delete_recursively | NOT RUN |
...
| TC-28b | Destroy connection | Removed | Not exercised: connection resource not run | NOT RUN |

### Error handling

| ID | Case | Expected | Actual | Status |
| --- | --- | --- | --- | --- |
| TC-29 | Invalid API token | Clear auth error, no partial state | Not exercised | NOT RUN |
| TC-30 | Role with neither `model_id` nor `connection_id` | Config validation error before any call | Not exercised | NOT RUN |
| TC-31 | Invalid `dialect` | Validation error listing allowed values | Not exercised | NOT RUN |
...
| TC-33 | Delete an object outside Terraform, then plan | Removed from state, recreate proposed | Not exercised | NOT RUN |
